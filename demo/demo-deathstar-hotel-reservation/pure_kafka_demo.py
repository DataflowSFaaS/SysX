...
def populate(sys_x_client: SyncSysXClient):
    # populate the geo table
    sys_x_client.send_event(operator=geo_operator,
                           key=1,
                           function='create',
                           params=(37.7867, 0))
    sys_x_client.send_event(operator=geo_operator,
    ...
    # populate rate
    sys_x_client.send_event(operator=rate_operator,
                           key=1,
                           function='create',
                           params=("RACK",
                                   "2015-04-09",
    ...
    sys_x_client.send_event(operator=recommendation_operator,
                           key=1,
                           function='create',
    ...
    for i in range(501):
        ...
        password = str(i) * 10
        sys_x_client.send_event(operator=user_operator,
                               key=username,
                               function='create',
                               params=(password,))
    ...
    # populate hotels
    for i in range(100):
        sys_x_client.send_event(operator=hotel_operator,
                               key=i,
                               function='create',
                               params=(10,))
    ...
    # populate flights
    for i in range(100):
        sys_x_client.send_event(operator=flight_operator,
                               key=i,
                               function='create',
                               params=(10,))
...
def deathstar_init(sys_x: SyncSysXClient):
    submit_graph(sys_x)
    time.sleep(60)
    populate(sys_x)
