...
def populate_warehouse(sys_x: SyncSysXClient):
    with open(os.path.join(script_path, "data/warehouse.csv"), "r") as f:
        ...
        for _, line in tqdm(enumerate(reader), desc="Populating Warehouse"):
            ...
            sys_x.send_event(operator=warehouse_operator,
                            key=warehouse_key,
                            function='insert',
                            params=(warehouse_data,))


def populate_district(sys_x: SyncSysXClient):
    with open(os.path.join(script_path, "data/district.csv"), "r") as f:
        ...
        for _, line in tqdm(enumerate(reader), desc="Populating District"):
            ...
            sys_x.send_event(operator=district_operator,
                            key=district_key,
                            function='insert',
                            params=(district_data,))


def populate_customer(sys_x: SyncSysXClient):
    c = 0
    customer_index_data = {}
    ...
    with open(os.path.join(script_path, "data/customer.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Customer"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=customer_operator,
                                           partition=partition,
        ...
        for i, (customer_idx_key, customer_idx_data) in enumerate(customer_index_data.items()):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=customer_idx_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_history(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/history.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating History"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=history_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_new_order(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/new_order.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating New Order"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=new_order_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_order(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/order.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Order"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=order_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_order_line(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/order_line.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Order Line"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=order_line_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_item(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/item.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Item"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=item_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_stock(sys_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/stock.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Stock"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    sys_x.send_batch_insert(operator=stock_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def submit_graph(sys_x: SyncSysXClient):
    ...
    sys_x.submit_dataflow(g)
    print("Graph submitted")


def tpc_c_init(sys_x: SyncSysXClient):
    submit_graph(sys_x)
    time.sleep(60)
    populate_warehouse(sys_x)
    sys_x.flush()
    populate_district(sys_x)
    sys_x.flush()
    populate_customer(sys_x)
    sys_x.flush()
    populate_history(sys_x)
    sys_x.flush()
    populate_new_order(sys_x)
    sys_x.flush()
    populate_order(sys_x)
    sys_x.flush()
    populate_order_line(sys_x)
    sys_x.flush()
    populate_item(sys_x)
    sys_x.flush()
    populate_stock(sys_x)
    sys_x.flush()
...
def benchmark_runner(proc_num) -> dict[bytes, dict]:
    ...
    sys_x = SyncSysXClient(SYSX_HOST, SYSX_PORT, kafka_url=KAFKA_URL)
    sys_x.open()
    ...
    for _ in range(seconds):
        ...
        for i in range(messages_per_second):
            ...
            request_id = sys_x.send_event(operator=operator,
                                         key=key,
                                         function=func_name,
                                         params=params)
...
def main():
    sys_x_client = SyncSysXClient(SYSX_HOST, SYSX_PORT, kafka_url=KAFKA_URL)

    sys_x_client.open()
    tpc_c_init(sys_x_client)
    sys_x_client.close()
