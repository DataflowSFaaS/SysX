...
def populate_warehouse(system_x: SyncSysXClient):
    with open(os.path.join(script_path, "data/warehouse.csv"), "r") as f:
        ...
        for _, line in tqdm(enumerate(reader), desc="Populating Warehouse"):
            ...
            system_x.send_event(operator=warehouse_operator,
                            key=warehouse_key,
                            function='insert',
                            params=(warehouse_data,))


def populate_district(system_x: SyncSysXClient):
    with open(os.path.join(script_path, "data/district.csv"), "r") as f:
        ...
        for _, line in tqdm(enumerate(reader), desc="Populating District"):
            ...
            system_x.send_event(operator=district_operator,
                            key=district_key,
                            function='insert',
                            params=(district_data,))


def populate_customer(system_x: SyncSysXClient):
    c = 0
    customer_index_data = {}
    ...
    with open(os.path.join(script_path, "data/customer.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Customer"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=customer_operator,
                                           partition=partition,
        ...
        for i, (customer_idx_key, customer_idx_data) in enumerate(customer_index_data.items()):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=customer_idx_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_history(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/history.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating History"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=history_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_new_order(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/new_order.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating New Order"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=new_order_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_order(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/order.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Order"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=order_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_order_line(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/order_line.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Order Line"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=order_line_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_item(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/item.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Item"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=item_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def populate_stock(system_x: SyncSysXClient):
    c = 0
    ...
    with open(os.path.join(script_path, "data/stock.csv"), "r") as f:
        ...
        for i, line in tqdm(enumerate(reader), desc="Populating Stock"):
            ...
            if i % 1_000 == 0 or i == num_lines - 1:
                for partition, kv_pairs in partitions.items():
                    system_x.send_batch_insert(operator=stock_operator,
                                           partition=partition,
                                           function='insert_batch',
                                           key_value_pairs=kv_pairs)
                partitions: dict[int, dict] = {p: {} for p in range(N_PARTITIONS)}
...
def tpc_c_init(system_x: SyncSysXClient):
    submit_graph(system_x)
    time.sleep(60)
    populate_warehouse(system_x)
    system_x.flush()
    populate_district(system_x)
    system_x.flush()
    populate_customer(system_x)
    system_x.flush()
    populate_history(system_x)
    system_x.flush()
    populate_new_order(system_x)
    system_x.flush()
    populate_order(system_x)
    system_x.flush()
    populate_order_line(system_x)
    system_x.flush()
    populate_item(system_x)
    system_x.flush()
    populate_stock(system_x)
    system_x.flush()
