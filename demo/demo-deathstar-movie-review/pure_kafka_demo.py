...
def populate_user(sys_x_client: SyncSysXClient):
    for i in range(1000):
        user_id = f'user{i}'
...
def populate_movie(sys_x_client: SyncSysXClient):
    for movie in movie_data:
        movie_id = movie["MovieId"]
        sys_x_client.send_event(operator=movie_info_operator,
                               key=movie_id,
                               function='write',
                               params=(movie,))

        sys_x_client.send_event(operator=plot_operator,
                               key=movie_id,
                               function='write',
                               params=("plot",))

        sys_x_client.send_event(operator=movie_id_operator,
                               key=movie["Title"],
