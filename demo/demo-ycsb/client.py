...
def ycsb_init(sys_x: SyncSysXClient, operator: Operator, keys: list[int]):
    submit_graph(sys_x)
    time.sleep(5)
    # INSERT
    ...
    for i in tqdm(keys):
        ...
        if i % 100_000 == 0 or i == len(keys) - 1:
            for partition, kv_pairs in partitions.items():
                sys_x.send_batch_insert(operator=operator,
                                       partition=partition,
...
def main():
    ...
    ycsb_init(sys_x_client, ycsb_operator, key_list)
    ...
    if run_with_validation:
        ...
        for key in key_list:
            request_id = sys_x_client.send_event(operator=ycsb_operator,
                                                key=key,
                                                function="read")
            validation_results[request_id] = {"op": f'read -> {key}'}

        sys_x_client.close()

        for request_id, metadata in sys_x_client.delivery_timestamps.items():
