...
from .system_x_future import SysXFuture
...
class SyncSysXClient(BaseSysXClient):
    ...
    def __init__(self,
        ...
        self._futures: dict[bytes, SysXFuture] = {}
    ...
    def start_consuming_results(self):
        ...
        while 'system_x-metadata' not in md.topics:
            print(f"Awaiting egress topic to be created by the SysX coordinator | topics: {md.topics}")
            time.sleep(1)
            md: ClusterMetadata = function_results_consumer.list_topics()
        topics_to_subscribe = ['system_x-metadata'] + [topic for topic in md.topics if topic.endswith('--OUT')]
    ...
    def send_event(self,
                   ...
                   function: Type | str,
                   params: tuple = tuple(),
                   serializer: Serializer = Serializer.MSGPACK) -> SysXFuture:
        request_id, serialized_value, partition = self._prepare_kafka_message(key,
                                                                              operator,
                                                                              function,
                                                                              params,
                                                                              serializer)
        self._futures[request_id] = SysXFuture(request_id=request_id)
    ...
    def submit_dataflow(self, stateflow_graph: StateflowGraph, external_modules: tuple = None):
        ...
        s = socket.socket()
        s.connect((self._system_x_coordinator_adr, self._system_x_coordinator_port))
        s.send(msg)
        s.close()
