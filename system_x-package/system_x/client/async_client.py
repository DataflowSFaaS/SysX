...
from .base_client import BaseSysXClient
from .system_x_future import SysXAsyncFuture
...
class AsyncSysXClient(BaseSysXClient):

    _kafka_producer: AIOKafkaProducer

    def __init__(self,
                 system_x_coordinator_adr: str,
                 system_x_coordinator_port: int,
                 kafka_url: str):
        super().__init__(system_x_coordinator_adr, system_x_coordinator_port)
        self._kafka_url = kafka_url
        self._futures: dict[bytes, SysXAsyncFuture] = {}
    ...
    async def start_consumer_task(self):
        ...
        while 'system_x-metadata' not in topics:
            topics = await self._background_consumer.topics()
            print("Awaiting egress topics to be created by the SysX coordinator")
            await asyncio.sleep(1)
        topics_to_subscribe = ['system_x-metadata'] + [topic for topic in topics if topic.endswith('--OUT')]
    ...
    async def send_event(self,
                         ...
                         function: Type | str,
                         params: tuple = tuple(),
                         serializer: Serializer = Serializer.MSGPACK) -> SysXAsyncFuture:
        request_id, serialized_value, partition = self._prepare_kafka_message(key,
                                                                              operator,
                                                                              function,
                                                                              params,
                                                                              serializer)
        self._futures[request_id] = SysXAsyncFuture(request_id=request_id)
    ...
    async def submit_dataflow(self, stateflow_graph: StateflowGraph, external_modules: tuple = None):
        ...
        await self._networking_manager.send_message(self._system_x_coordinator_adr,
                                                    self._system_x_coordinator_port,
                                                    msg=(stateflow_graph, ),
